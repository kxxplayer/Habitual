...
export const generateHabitProgramFromGoal = ai.defineFlow(
  ...
  async ({ goal, focusDuration }) => {
    ...
    const prompt = `
      ...
      **Instructions:** Create an inspiring program name and an array of 3-5 distinct habits. For each habit, provide a name, description, category, daysOfWeek, and optional timing/duration details.
...
export const getReflectionStarter = ai.defineFlow(
  {
    ...
    outputSchema: z.object({ reflectionPrompt: z.string() }), // Matches genkit-service.ts
  },
  async ({ habitName }) => {
    const prompt = `Generate one thoughtful, open-ended reflection question for the habit: "${habitName}".`;
    const { text } = await ai.generate({ model, prompt });
    return { reflectionPrompt: text }; // Matches genkit-service.ts
  }
);
...
export const getCommonHabitSuggestions = ai.defineFlow(
  ...
  async ({ category }) => {
    const prompt = `List 5 common, beginner-friendly habits for the category "${category}". Respond with ONLY a JSON object containing a "suggestions" array.`;
    const { text } = await ai.generate({ model, prompt });
    try {
        const parsed = JSON.parse(text.replace(/```json|```/g, ''));
        return parsed;
