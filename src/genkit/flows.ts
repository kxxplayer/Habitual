  ...
  async ({ description }) => {
    ...
    const prompt = `
      ...
      **Instructions:**
      ...
      3.  **daysOfWeek**: Extract days. "Weekends" are ["Sat", "Sun"]. "Weekdays" are ["Mon", "Tue", "Wed", "Thu", "Fri"]. "Daily" is all seven days.
      4.  **duration**: Extract duration. "1 hour" means "durationHours": 1. "30 minutes" means "durationMinutes": 30.
      5.  **optimalTiming**: Infer a general time like "Morning", "Afternoon", or "Evening".
      6.  **specificTime**: If a time like "8:00 AM" or "17:00" is mentioned, use HH:mm format.

      **User Description:** "${description}"

      **Output ONLY the JSON object.**

      **Example for "do 5 sql questions on weekends for 1 hour":**
      {
    ...
    try {
      // FIX: Call text as a function `text()` to get the string response.
      ...
      // FIX: Also call text() in the error log.
    ...
    const prompt = `
      ...
      **Instructions:**
      ...
      2.  **suggestedHabits**: Generate an array of 3 to 5 distinct, complementary habits. For each habit:
          ...
          * **daysOfWeek**: A recommended weekly schedule using these exact values: "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat".

      **Respond with ONLY the valid JSON object.**

      **Example for "Learn Python in 3 months":**
      {
        "programName": "Python Mastery: 3-Month Sprint",
        "suggestedHabits": [
          {
            "name": "Daily Coding Challenge",
            "description": "Solve one problem on a platform like LeetCode or HackerRank to build problem-solving muscle.",
            "category": "Work & Study",
            "daysOfWeek": ["Mon", "Tue", "Wed", "Thu", "Fri"]
          },
          {
            "name": "Build a Mini-Project",
            "description": "Dedicate time to a personal Python project to apply what you've learned in a practical way.",
            "category": "Creative",
            "daysOfWeek": ["Sat", "Sun"]
          },
          {
...
Keep the tip concise and encouraging.`;
    const { text } = await ai.generate({ model, prompt });
    // FIX: Call text as a function `text()`
...
export const getCommonHabitSuggestions = ai.defineFlow(
  ...
  async ({ category }) => {
    const prompt = `List 5 common, beginner-friendly habits for the category "${category}". Respond with ONLY a JSON object containing a "suggestions" array. Each item should have a "name" and a "category".`;
    const { text } = await ai.generate({ model, prompt });
    // FIX: Call text as a function `text()`
