...
const RescheduleMissedHabitDialog: FC<RescheduleMissedHabitDialogProps> = ({
  isOpen,
  onClose,
  habitName,
  originalMissedDate,
  ...
  const handleReschedule = () => {
    if (selectedDate) {
      onReschedule(format(selectedDate, 'yyyy-MM-dd'));
      onClose();
    }
  };
  ...
  try {
    formattedOriginalDate = format(parseISO(originalMissedDate + 'T00:00:00Z'), 'MMMM d, yyyy');
  } catch (e) {
    console.error("Error formatting original missed date:", e);
  }
      ...
      <DialogContent className="sm:max-w-[480px] bg-card rounded-lg shadow-xl">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold flex items-center">
            ...
            Action for Missed Habit
          </DialogTitle>
          <DialogDescription>
            Habit: "{habitName}" was missed on {formattedOriginalDate}.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4 px-1 space-y-3">
          <div>
            ...
            <Calendar
              ...
              disabled={(date) => isBefore(date, today)} // Disable past dates and today
              initialFocus
              className="rounded-md border bg-input/20 w-full flex justify-center"
            />
          </div>
        </div>
        ...
        <DialogFooter className="pt-4 flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
          <Button onClick={handleSkip} variant="outline" className="w-full sm:w-auto">
            <SkipForward className="mr-2 h-4 w-4" /> Mark as Skipped
          </Button>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-end sm:space-x-2 w-full sm:w-auto">
            <DialogClose asChild>
              <Button type="button" variant="ghost" className="w-full sm:w-auto" onClick={() => setSelectedDate(undefined)}>
                Cancel
              </Button>
            </DialogClose>
            <Button onClick={handleReschedule} disabled={!selectedDate} className="w-full sm:w-auto">
              Reschedule
            </Button>
          </div>
        </DialogFooter>
