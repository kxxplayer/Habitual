...
const AISuggestionDialog: FC<AISuggestionDialogProps> = ({
  isOpen,
  onClose,
  habitName,
  suggestion,
      ...
      <DialogContent className="sm:max-w-[480px] bg-card rounded-lg shadow-xl">
        <DialogHeader>
          <DialogTitle className="text-2xl font-semibold flex items-center">
            ...
            AI Suggestion for "{habitName}"
          </DialogTitle>
          <DialogDescription>
            Here's a personalized tip to help you stay on track.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4 px-2 min-h-[100px]">
          ...
          {error && !isLoading && (
            <div className="text-destructive text-center">
              <p>Could not load suggestion: {error}</p>
            </div>
          )}
          {!isLoading && !error && suggestion && (
           ...
           {!isLoading && !error && !suggestion && (
            <p className="text-muted-foreground text-center">No suggestion available at the moment.</p>
          )}
        </div>
